...
///////////////////////////////////////////////////////////////////////////////
__global__ void ScaleDownDenseShift(float *d_Result, float *d_Data, int width,
                                    int pitch, int height, int newpitch) {
...
__global__ void ScaleDownDense(float *d_Result, float *d_Data, int width,
                               int pitch, int height, int newpitch) {
#define BW (SCALEDOWN_W + 4)
#define BH (SCALEDOWN_H + 4)
#define W2 (SCALEDOWN_W / 2)
...
__global__ void ComputeOrientationsCONSTNew(float *image, int w, int p, int h,
...
#define LEN 32  //%%%% Note: Lowe suggests 36, not 32
    ...
    for (int bx = blockIdx.x + fstPts; bx < totPts; bx += gridDim.x) {
        ...
        for (int i = tx; i < (WID - 6) * (WID - 6); i += blockDim.x) {
            ...
            int bin =
                (int)((LEN / 2) * atan2f(dy, dx) / 3.1416f + (LEN / 2) + 0.5f) %
                LEN;
        ...
        if (tx < LEN) {
            ...
            hist[tx] =
                (val > hist[x1m + LEN] && val >= hist[x1p + LEN] ? val : 0.0f);
        }
        __syncthreads();
        if (tx == 0) {
            float maxval1 = 0.0;
...
__global__ void FindPointsMultiTest(float *d_Data0, SiftPoint *d_Sift,
                                    int width, int pitch, int height,
                                    float subsampling, float lowestScale,
                                    float thresh, float factor, float edgeLimit,
                                    int octave) {
...
#define MEMWID (MINMAX_W + 2)
    ...
    for (int y = 0; y < yloops; y++) {
        ...
        if (__any_sync(0xffffffff, fabs(d11) > thresh)) {
            ...
            if (tx > 0 && tx < MINMAX_W + 1 && xpos <= maxx)
                ptbits |=
                    ((d11 < fminf(-thresh, minv)) | (d11 > fmaxf(thresh, maxv)))
                    << y;
        }
    }
...
__global__ void FindPointsMultiOld(float *d_Data0, SiftPoint *d_Sift, int width,
                                   int pitch, int height, float subsampling,
                                   float lowestScale, float thresh,
                                   float factor, float edgeLimit, int octave) {
#define MEMWID (MINMAX_W + 2)
    ...
    for (int y = 0; y < yloops; y++) {
        ...
        if (tx > 0 && tx < MINMAX_W + 1 && xpos <= maxx) {
            if (d21 < -thresh) {
                float minv =
                    fminf(fminf(fminf(ymin2[tx - 1], ymin2[tx + 1]), ymin1[tx]),
                          ymin3[tx]);
                minv = fminf(fminf(minv, d20), d22);
                if (d21 < minv) {
                    int pos = atomicInc(&cnt, MEMWID - 1);
            ...
            if (d21 > thresh) {
                float maxv =
                    fmaxf(fmaxf(fmaxf(ymax2[tx - 1], ymax2[tx + 1]), ymax1[tx]),
                          ymax3[tx]);
                maxv = fmaxf(fmaxf(maxv, d20), d22);
                if (d21 > maxv) {
                    int pos = atomicInc(&cnt, MEMWID - 1);
...
__global__ void LaplaceMultiTex(cudaTextureObject_t texObj, float *d_Result,
...
__global__ void LaplaceMultiMemWide(float *d_Image, float *d_Result, int width,
                                    int pitch, int height, int octave) {
    __shared__ float buff[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
    const int tx = threadIdx.x;
    const int xp = blockIdx.x * LAPLACE_W + tx;
    ...
    if (tx < LAPLACE_W / 4 && xp4 < width) {
        ...
        for (int scale = 1; scale < LAPLACE_S; scale++) {
            ...
            reinterpret_cast<float4 *>(
                &d_Result[(scale - 1) * height * pitch + yp * pitch + xp4])[0] =
                dif4;
            old4 = new4;
        }
    }
...
__global__ void LaplaceMultiMemTest(float *d_Image, float *d_Result, int width,
                                    int pitch, int height, int octave) {
    __shared__ float data1[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
    __shared__ float data2[LAPLACE_W * LAPLACE_S];
    const int tx = threadIdx.x;
...
__global__ void LaplaceMultiMemOld(float *d_Image, float *d_Result, int width,
                                   int pitch, int height, int octave) {
    __shared__ float data1[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
    __shared__ float data2[LAPLACE_W * LAPLACE_S];
    const int tx = threadIdx.x;
...
__global__ void LowPass(float *d_Image, float *d_Result, int width, int pitch,
                        int height) {
...
__global__ void LowPassBlockOld(float *d_Image, float *d_Result, int width,
