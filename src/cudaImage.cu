...
double CudaImage::CopyToTexture(CudaImage &dst, bool host) {
    ...
    if (host)
        ...
        safeCall(cudaMemcpy2DToArray((cudaArray *)dst.t_data, 0, 0, h_data,
                                     sizeof(float) * pitch * dst.height,
                                     sizeof(float) * pitch * dst.height, 1,
                                     cudaMemcpyHostToDevice));
    else
        // safeCall(cudaMemcpyToArray((cudaArray *)dst.t_data, 0, 0, d_data,
        // sizeof(float)*pitch*dst.height, cudaMemcpyDeviceToDevice));
        safeCall(cudaMemcpy2DToArray((cudaArray *)dst.t_data, 0, 0, d_data,
                                     sizeof(float) * pitch * dst.height,
                                     sizeof(float) * pitch * dst.height, 1,
