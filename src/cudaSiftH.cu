...
void InitCuda(int devNum) {
    ...
    printf("Cuda Memory Info: FREE: %f TOTAL: %f\n", free / 1048576.0,
           total / 1048576.0);
#ifdef MANAGEDMEM
    printf("Using managed memory\n");
#endif
}
...
double ExtractSiftAsync(SiftData &siftData, CudaImage &img, int numOctaves,
    ...
    safeCall(cudaMemsetAsync(d_PointCounterAddr, 0, (8 * 2 + 1) * sizeof(int),
                             cuda_stream));
    ...
    if (!scaleUp) {
        ...
        safeCall(cudaMemcpyToSymbolAsync(d_LaplaceKernel, kernel,
                                         8 * 12 * 16 * sizeof(float), 0,
                                         cudaMemcpyHostToDevice, cuda_stream));
        LowPassAsync(lowImg, img, max(initBlur, 0.001f), cuda_stream);
        TimerGPU timer1(0);
        ExtractSiftLoopAsync(
            siftData, lowImg, numOctaves, 0.0f, thresh, lowestScale, 1.0f,
            memoryTmp, memorySub + height * iAlignUp(width, 128), cuda_stream);
        safeCall(cudaMemcpyAsync(
            &siftData.numPts, &d_PointCounterAddr[2 * numOctaves], sizeof(int),
...
double ExtractSift(SiftData &siftData, CudaImage &img, int numOctaves,
                   double initBlur, float thresh, float lowestScale,
                   bool scaleUp, float *tempMemory) {
    TimerGPU timer(0);
    unsigned int *d_PointCounterAddr;
    ...
    if (!scaleUp) {
        float kernel[8 * 12 * 16];
        PrepareLaplaceKernels(numOctaves, 0.0f, kernel);
        safeCall(cudaMemcpyToSymbol(d_LaplaceKernel, kernel,
...
int ExtractSiftLoopAsync(SiftData &siftData, CudaImage &img, int numOctaves,
    ...
    if (numOctaves > 1) {
        ...
        ExtractSiftLoopAsync(siftData, subImg, numOctaves - 1, totInitBlur,
                             thresh, lowestScale, subsampling * 2.0f, memoryTmp,
                             memorySub + (h / 2) * p, cuda_stream);
    }
    ExtractSiftOctaveAsync(siftData, img, numOctaves, thresh, lowestScale,
                           subsampling, memoryTmp, cuda_stream);
#ifdef VERBOSE
    double totTime = timer.read();
...
void ExtractSiftOctaveAsync(SiftData &siftData, CudaImage &img, int octave,
    ...
    safeCall(cudaMemcpyAsync(&fstPts, &d_PointCounterAddr[2 * octave - 1],
    ...
    texDesc.filterMode =
        cudaFilterModeLinear;  // what happens if I use FilterModePoint?
    ...
    LaplaceMultiAsync(texObj, img, diffImg, octave, cuda_stream);
    FindPointsMultiAsync(diffImg, siftData, thresh, 10.0f, 1.0f / NUM_SCALES,
    ...
    TimerGPU timer4;
#endif
    ComputeOrientationsAsync(texObj, img, siftData, octave, cuda_stream);
    ExtractSiftDescriptorsAsync(texObj, siftData, subsampling, octave,
                                cuda_stream);
...
double ComputeOrientationsAsync(cudaTextureObject_t texObj, CudaImage &src,
    ...
    dim3 threads(11 * 11);
#ifdef MANAGEDMEM
    ComputeOrientationsCONST<<<blocks, threads, 0, cuda_stream>>>(
        texObj, siftData.m_data, octave);
#else
    ComputeOrientationsCONST<<<blocks, threads, 0, cuda_stream>>>(
        texObj, siftData.d_data, octave);
#endif
...
double ExtractSiftDescriptorsAsync(cudaTextureObject_t texObj,
    ...
    dim3 threads(16, 8);
#ifdef MANAGEDMEM
    ExtractSiftDescriptorsCONST<<<blocks, threads, 0, cuda_stream>>>(
        texObj, siftData.m_data, subsampling, octave);
#else
    ExtractSiftDescriptorsCONSTNew<<<blocks, threads, 0, cuda_stream>>>(
        texObj, siftData.d_data, subsampling, octave);
#endif
...
double RescalePositions(SiftData &siftData, float scale) {
    ...
    dim3 threads(64);
#ifdef MANAGEDMEM
    RescalePositions<<<blocks, threads>>>(siftData.m_data, siftData.numPts,
                                          scale);
#else
...
double LowPassAsync(CudaImage &res, CudaImage &src, float scale,
    ...
    LowPassBlock<<<blocks, threads, 0, cuda_stream>>>(src.d_data, res.d_data,
                                                      width, pitch, height);
#else
...
void PrepareLaplaceKernels(int numOctaves, float initBlur, float *kernel) {
    ...
    for (int i = 0; i < NUM_SCALES + 3; i++) {
        ...
        for (int j = 0; j <= LAPLACE_R; j++) {
            ...
            kernelSum +=
                (j == 0 ? 1 : 2) * kernel[numOctaves * 12 * 16 + 16 * i + j];
        }
        for (int j = 0; j <= LAPLACE_R; j++)
...
double LaplaceMultiAsync(cudaTextureObject_t texObj, CudaImage &baseImage,
                         ...
                         cudaStream_t cuda_stream) {
    // get width, pitch and height
    int width = results[0].width;
    int pitch = results[0].pitch;
    int height = results[0].height;
#if 1
    // numthreads per block is 128 + 2*Kernel Radius
    dim3 threads(LAPLACE_W + 2 * LAPLACE_R);
    // num blocks is ceil(width,128), height
    dim3 blocks(iDivUp(width, LAPLACE_W), height);

    LaplaceMultiMem<<<blocks, threads, 0, cuda_stream>>>(
        baseImage.d_data, results[0].d_data, width, pitch, height, octave);
#endif
#if 0
  ...
  LaplaceMultiMemOld<<<blocks, threads>>>(baseImage.d_data, results[0].d_data, width, pitch, height, octave);
    ...
    LaplaceMultiTex<<<blocks, threads>>>(texObj, results[0].d_data, width,
                                         pitch, height, octave);
#endif
...
#ifdef MANAGEDMEM
    FindPointsMultiNew<<<blocks, threads, 0, cuda_stream>>>(
        sources->d_data, siftData.m_data, w, p, h, subsampling, lowestScale,
        thresh, factor, edgeLimit, octave);
#else
    FindPointsMultiNew<<<blocks, threads, 0, cuda_stream>>>(
        sources->d_data, siftData.d_data, w, p, h, subsampling, lowestScale,
        thresh, factor, edgeLimit, octave);
#endif
#endif
